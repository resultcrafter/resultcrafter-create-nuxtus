...
if (!commandExistsSync('git')) {
  console.log(
    chalk.red(
      'Git is required to install Nuxtus. Please install Git and try again.',
    ),
  );
  process.exit(1);
}
...
async function main(): Promise<void> {
  try {
    ...
    type QuestionFunction = (params: {
      client: string;
      filepath: string;
    }) => unknown;
    credentials = await inquirer.prompt(
      (databaseQuestions[dbClient] as QuestionFunction[]).map(
  ...
  function taskDone(name: string): void {
    pending.delete(name);
    if (pending.size > 0) {
  ...
  Promise.all([directus, nuxt, cleanup]).then(() => {
    ...
    if (templateName === 'premium') {
      console.log(
        chalk.green('🚀 ResultCrafter Premium site is ready!\n\n') +
